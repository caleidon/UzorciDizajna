...
{
    public RasporedCsvCitac(Model model) : base(model) { }
                                                ...
                                                out int indexStupca)
    {
        ...
        Vez vez = BrodskaLuka.Instance.DajSveVezove().FirstOrDefault(v => v.Id == idVeza) ??
                  throw new StavkaNijePronadenaException($"Vez {idVeza} ne postoji.");
        ...
        if (!vez.PodrzavaBrod(brod)) { throw new PogresniPodaciException($"Vez {vez.Id} ne podržava brod {brod.Id}."); }
        ...
        if (daniUTjednuBrojevi.Count != daniUTjednuBrojevi.Distinct().Count() || daniUTjednuBrojevi.Count > 7) {
            throw new PogresniPodaciException("Dani u tjednu moraju biti jedinstveni i ne smije ih biti više od 7.");
        }
        ...
        if (postojeciPodaciLista.Any(r =>
            ...
            throw new PogresniPodaciException("Nije moguce imati isti vremenski period sa razlicitim danima u tjednu.");
        }
        ...
        if (postojeciPodaciLista.Any(r =>
                ...
                r.VrijemeOd == vrijemeOd &&
                r.VrijemeDo == vrijemeDo)) {
            throw new PogresniPodaciException("Nije moguće imati više rasporeda u istom vremenskom intervalu.");
        }
