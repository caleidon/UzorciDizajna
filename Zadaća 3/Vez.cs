...
public class Vez : Leaf, IZauzetiVez
{
    ...
    public List<IZauzetost> SveZauzetosti { get; set; } = new();
    ...
    public void OslobodiPrivezaniBrod()
    {
        ...
        _model.IspisiPotvrdu($"Brod {PrivezaniBrod.Id} je otpušten sa veza {Id}.", true);
    ...
    public void UkloniZastarjeleRezervacije()
    {
        for (int index = SveZauzetosti.Count - 1; index >= 0; index--) {
            ...
            _model.IspisiPotvrdu($"Rezervacija veza {Id} od {rez.RezerviranOd} do {rez.RezerviranDo} je istekla.",
                false);
