...
public class Brod : ISubscriber, IHitniPrijevozHandler
{
    ...
    public bool JePrivezan => BrodskaLuka.Instance.SviVezovi.Any(v => v.PrivezaniBrod == this);
    ...
    public void PrimiPoruku(PorukaVhfKanala poruka, IMediator kanal)
    {
        ...
        if (olk.Odobreno) {
            Alati.IspisiPotvrdu($"Brod {Id} je dobio odobrenje za ulazak u luku.");
            return;
        }

        Alati.IspisiGresku($"Brod {Id} nije dobio odobrenje za ulazak u luku.");
    }
    ...
    public void HandleHitniPrijevoz(HitniPrijevoz hitniPrijevoz)
    {
        if (hitniPrijevoz.PreostaloSluzbenika > 0 && ZeliPrevestiSluzbenike) {
            ...
            Alati.IspisiPotvrdu($"Brod {Id} je će prevesti {brojSluzbenika} službenika.");
        }
        ...
        if (hitniPrijevoz.PreostaloImigranta > 0 && ZeliPrevestiImigrante) {
            ...
            Alati.IspisiPotvrdu($"Brod {Id} je će prevesti {brojImigranta} imigranta.");
        }
    ...
    {
        if (JePrivezan) {
            Vez privezaniVez = BrodskaLuka.Instance.SviVezovi.First(v => v.PrivezaniBrod == this);
